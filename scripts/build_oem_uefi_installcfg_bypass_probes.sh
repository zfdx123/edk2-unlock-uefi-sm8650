...
# Patch out the InstallConfigurationTable() call at 0xa700e810 by forcing EFI_SUCCESS in x0.
skip_payload="${ARTIFACT_DIR}/oem-uefi-probes/pineapple-oem-uefi-skip-installcfg.elf"
python3 "${ROOT_DIR}/scripts/patch_elf_address_probe.py" \
  --input "${PAYLOAD}" \
  --address "0xa700e810" \
  --mode success \
  --output "${skip_payload}"
build_boot_from_payload "${skip_payload}" "pineapple-oem-uefi-skip-installcfg"

# After bypassing InstallConfigurationTable(), force a reset at the first post-call helper.
skip_then_reset_payload="${ARTIFACT_DIR}/oem-uefi-probes/pineapple-oem-uefi-skip-installcfg-post-reset.elf"
python3 "${ROOT_DIR}/scripts/patch_elf_address_probe.py" \
  --input "${skip_payload}" \
  --address "0xa700e8a8" \
  --mode reset \
  --output "${skip_then_reset_payload}"
build_boot_from_payload "${skip_then_reset_payload}" "pineapple-oem-uefi-skip-installcfg-post-reset"
