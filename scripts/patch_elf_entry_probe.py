#!/usr/bin/env python3
"""Patch an ELF entrypoint with a tiny AArch64 probe stub."""
...
def entry_file_offset(data: bytes) -> int:
    elf_class = data[4]
    if elf_class == 2:
    ...
    raise ValueError("unsupported ELF or entrypoint not found in LOAD segment")


def build_stub(mode: str) -> bytes:
    with tempfile.TemporaryDirectory(prefix="elf-entry-probe-", dir="/tmp") as tmp_dir_name:
...
def main() -> int:
    ...
    patch_offset = entry_file_offset(data)
